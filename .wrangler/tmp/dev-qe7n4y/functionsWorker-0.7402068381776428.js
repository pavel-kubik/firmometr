...
// .wrangler/tmp/bundle-IVTQf2/checked-fetch.js
...
// .wrangler/tmp/pages-E818ob/functionsWorker-0.7402068381776428.mjs
...
var init_checked_fetch = __esm({
  "../.wrangler/tmp/bundle-Yl2Yn5/checked-fetch.js"() {
    "use strict";
...
var require_util = __commonJS({
  "../node_modules/fast-xml-parser/src/util.js"(exports) {
    "use strict";
    init_functionsRoutes_0_13781337098352953();
    init_checked_fetch();
...
var require_OptionsBuilder = __commonJS({
  "../node_modules/fast-xml-parser/src/xmlparser/OptionsBuilder.js"(exports) {
    ...
    var defaultOnDangerousProperty = /* @__PURE__ */ __name2((name) => {
    ...
    __name2(validatePropertyName, "validatePropertyName");
    ...
    __name2(normalizeProcessEntities, "normalizeProcessEntities");
...
var require_DocTypeReader = __commonJS({
  "../node_modules/fast-xml-parser/src/xmlparser/DocTypeReader.js"(exports, module) {
    ...
    var skipWhitespace = /* @__PURE__ */ __name2((data, index) => {
...
var require_OrderedObjParser = __commonJS({
  "../node_modules/fast-xml-parser/src/xmlparser/OrderedObjParser.js"(exports, module) {
    ...
    var OrderedObjParser = class {
      ...
      constructor(options) {
        ...
        this.htmlEntities = {
          ...
          "num_dec": { regex: /&#([0-9]{1,7});/g, val: /* @__PURE__ */ __name2((_, str2) => fromCodePoint(str2, 10, "&#"), "val") },
          "num_hex": { regex: /&#x([0-9a-fA-F]{1,6});/g, val: /* @__PURE__ */ __name2((_, str2) => fromCodePoint(str2, 16, "&#x"), "val") }
        };
    ...
    var replaceEntitiesValue = /* @__PURE__ */ __name2(function(val, tagName, jPath) {
      if (val.indexOf("&") === -1) {
        return val;
      }
    ...
    __name2(fromCodePoint, "fromCodePoint");
...
var require_json2xml = __commonJS({
  "../node_modules/fast-xml-parser/src/xmlbuilder/json2xml.js"(exports, module) {
    ...
    var defaultOptions = {
      ...
      tagValueProcessor: /* @__PURE__ */ __name2(function(key, a) {
        return a;
      }, "tagValueProcessor"),
      attributeValueProcessor: /* @__PURE__ */ __name2(function(attrName, a) {
        return a;
      }, "attributeValueProcessor"),
      preserveOrder: false,
      commentPropName: false,
...
__name(checkCap, "checkCap");
function resetCap(req) {
  const raw = (req.headers.get("cookie") ?? "").match(/(?:^|;)\s*srch_cnt=([^;]*)/)?.[1] ?? "";
  const [countStr, expiresStr] = raw.split("|");
  const now = Date.now();
  const expires = parseInt(expiresStr ?? "0", 10) || 0;
...
__name(resetCap, "resetCap");
function withCap(body, cap, status = 200) {
  ...
  if (status === 429 && cap.cookieValue) {
    ...
    headers["Retry-After"] = String(Math.max(0, Math.ceil((expires - Date.now()) / 1e3)));
  }
  return new Response(JSON.stringify(body), { status, headers });
}
__name(withCap, "withCap");
var FREE_CAP;
var WINDOW_MINUTES;
var WINDOW_MS;
var init_cap = __esm({
  "_shared/_cap.ts"() {
    ...
    FREE_CAP = 20;
    WINDOW_MINUTES = 5;
    WINDOW_MS = WINDOW_MINUTES * 6e4;
    __name2(checkCap, "checkCap");
    __name2(resetCap, "resetCap");
    __name2(withCap, "withCap");
  }
});
function isActive(r) {
  if (r.datumPmUkonceniUpadku) return false;
  const s = r.druhStavKonkursu;
  if (s === "MYLN\xDD Z\xC1P." || s === "PRAVOMOCNA" || s === "ODSKRTNUTA") return false;
  if (s && ACTIVE_STATI.has(s)) return true;
  return true;
}
__name(isActive, "isActive");
function assessClearance(records) {
  ...
  return { clarity: pastDirect.length > 0 ? "PAST_DEBTOR" : "CLEAR", activeSet: /* @__PURE__ */ new Set() };
}
__name(assessClearance, "assessClearance");
function stavKodFrom(data) {
...
__name(stavKodFrom, "stavKodFrom");
function stavNazevFrom(kod) {
  if (!kod) return null;
  if (kod === "AKTIVNI") return "Aktivn\xED";
  if (kod === "ZANIKLY" || kod === "ZANIKL\xDD") return "Zanikl\xFD";
  return kod;
}
__name(stavNazevFrom, "stavNazevFrom");
...
__name(fetchAres, "fetchAres");
async function fetchIsir(ico) {
...
__name(fetchIsir, "fetchIsir");
function parseIsirResponse(xml) {
  const parser = new import_fast_xml_parser.XMLParser({ ignoreAttributes: false, textNodeName: "_text" });
...
__name(parseIsirResponse, "parseIsirResponse");
async function fetchDph(dic) {
  const body = `<?xml version="1.0" encoding="UTF-8"?>
<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/"
                  xmlns:ns1="${DPH_NS}">
  <soapenv:Header/>
...
__name(fetchDph, "fetchDph");
function parseDphResponse(xml) {
  const parser = new import_fast_xml_parser.XMLParser({ ignoreAttributes: false, attributeNamePrefix: "@_", textNodeName: "_text" });
  const doc = parser.parse(xml);
  ...
  const ucty = ucetNodes.flatMap((ucet) => {
...
__name(parseDphResponse, "parseDphResponse");
async function fetchOrVr(ico) {
  try {
    let memberToStatutar2 = /* @__PURE__ */ __name(function(m) {
      ...
      if (fo) {
        const parts = [fo.titulPredJmenem, fo.jmeno, fo.prijmeni, fo.titulZaJmenem].filter((v) => v != null && String(v).trim() !== "").map((v) => String(v).trim());
        return {
    ...
    var memberToStatutar = memberToStatutar2;
    __name2(memberToStatutar2, "memberToStatutar");
    ...
    const primary = zaznamy.find((z) => z.primarniZaznam) ?? zaznamy[0];
    if (!primary) return { spisovatel: null, statutari: [] };
    const szArr = Array.isArray(primary.spisovaZnacka) ? primary.spisovaZnacka : [];
    const sz = szArr[0];
    const spisovatel = sz ? `${sz.oddil} ${sz.vlozka}` : null;
    const organy = primary.statutarniOrgany ?? [];
    const allMembers = organy.flatMap((o) => o.clenoveOrganu ?? []);
    const statutari = allMembers.map(memberToStatutar2);
    return { spisovatel, statutari };
  } catch {
    return { spisovatel: null, statutari: [] };
  }
}
__name(fetchOrVr, "fetchOrVr");
async function fetchOrSubjektId(ico) {
  try {
...
__name(fetchOrSubjektId, "fetchOrSubjektId");
async function fetchSbirkaListin(subjektId) {
  try {
    ...
    const html = await res.text();
    const countMatch = html.match(/z\s+(\d+)\s+(?:záznam[ůu]|listiny)/i) ?? html.match(/[Cc]elkem[:\s]+(\d+)/);
...
__name(fetchSbirkaListin, "fetchSbirkaListin");
async function fetchCuzk(addressText) {
  try {
...
var init_ico = __esm({
  "api/v1/search/ico/[ico].ts"() {
    ...
    ARES_VR_BASE = "https://ares.gov.cz/ekonomicke-subjekty-v-be/rest";
    OR_PORTAL = "https://or.justice.cz/ias/ui";
    ISIR_CUZK = "https://isir.justice.cz:8443/isir_cuzk_ws/IsirWsCuzkService";
    CUZK_BASE = "https://ags.cuzk.gov.cz/arcgis/rest/services/RUIAN/Vyhledavac_adres_sluzby/MapServer/exts/VyhledavacAdresRestSoe/FindAddress";
    CUZK_NS = "http://isirws.cca.cz/types/";
    DPH_SOAP = "https://adisrws.mfcr.cz/dpr/axis2/services/rozhraniCRPDPH.rozhraniCRPDPHSOAP";
    DPH_NS = "http://adis.mfcr.cz/rozhraniCRPDPH/";
    ACTIVE_STATI = /* @__PURE__ */ new Set([
      "NEVYRIZENA",
      "OBZIVLA",
      "VYRIZENA",
      "KONKURS",
    ...
    INVALID_STATI = /* @__PURE__ */ new Set(["MYLN\xDD Z\xC1P.", "ODSKRTNUTA"]);
    __name2(isActive, "isActive");
    __name2(assessClearance, "assessClearance");
    __name2(stavKodFrom, "stavKodFrom");
    __name2(stavNazevFrom, "stavNazevFrom");
    __name2(fetchAres, "fetchAres");
    __name2(fetchIsir, "fetchIsir");
    __name2(parseIsirResponse, "parseIsirResponse");
    DPH_NOT_PLATCE = { isPlatce: false, nespolehlivy: null, datumNespolehlivosti: null, ucty: [], nedostupne: false };
    DPH_UNAVAILABLE = { isPlatce: false, nespolehlivy: null, datumNespolehlivosti: null, ucty: [], nedostupne: true };
    __name2(fetchDph, "fetchDph");
    __name2(parseDphResponse, "parseDphResponse");
    __name2(fetchOrVr, "fetchOrVr");
    __name2(fetchOrSubjektId, "fetchOrSubjektId");
    __name2(fetchSbirkaListin, "fetchSbirkaListin");
    __name2(fetchCuzk, "fetchCuzk");
    __name2(findNode, "findNode");
    __name2(collectNodes, "collectNodes");
    __name2(str, "str");
    onRequest = /* @__PURE__ */ __name2(async ({ request, params }) => {
      ...
      const [aresData, isirRecords, orVr, orSubjektId] = await Promise.all([
        ...
        fetchIsir(ico),
        fetchOrVr(ico),
        fetchOrSubjektId(ico)
      ]);
      const dic = aresData?.dic ?? "CZ" + ico.padStart(8, "0");
      const sidloText = aresData?.sidlo?.textovaAdresa ?? null;
      ...
      const proceedings = isirRecords.map((r) => {
        const parts = [r.cisloSenatu, r.druhVec, [r.bcVec, r.rocnik].filter(Boolean).join("/")].filter(Boolean);
        return {
      ...
      return withCap({
        ...
        or: hasOrData ? {
          ...
          orUrl: orSubjektId ? `https://or.justice.cz/ias/ui/rejstrik-firma.vysledky?subjektId=${orSubjektId}&typ=PLATNY` : null
        } : null,
        isWatched: false
      }, cap);
    }, "onRequest");
...
var init_cap_reset = __esm({
  "api/v1/cap-reset.ts"() {
    ...
    onRequest2 = /* @__PURE__ */ __name2(async ({ request }) => {
      ...
      const expires = parseInt(cap.cookieValue.split("|")[1] ?? "0", 10) || 0;
      const retryAfter = Math.max(0, Math.ceil((expires - Date.now()) / 1e3));
      return withCap({ retryAfter, blocked: cap.blocked }, cap, 200);
    }, "onRequest");
  }
});
function stavNazevFrom2(kod) {
  if (!kod) return null;
...
__name(stavNazevFrom2, "stavNazevFrom2");
function stavKodFrom2(data) {
  const reg = data?.seznamRegistraci;
  const raw = reg?.stavZdrojeRos ?? reg?.stavZdrojeVr ?? reg?.stavZdrojeRes ?? null;
  return raw === "NEEXISTUJICI" ? null : raw ?? null;
}
__name(stavKodFrom2, "stavKodFrom2");
var ARES_BASE2;
var onRequest3;
var init_search = __esm({
  "api/v1/search.ts"() {
    ...
    init_checked_fetch();
    init_cap();
    ARES_BASE2 = "https://ares.gov.cz/ekonomicke-subjekty-v-be/rest";
    __name2(stavNazevFrom2, "stavNazevFrom");
    __name2(stavKodFrom2, "stavKodFrom");
    onRequest3 = /* @__PURE__ */ __name2(async ({ request }) => {
      ...
      if (cap.blocked) return withCap({ error: "limit_reached" }, cap, 429);
      try {
        const res = await fetch(`${ARES_BASE2}/ekonomicke-subjekty/vyhledat`, {
          method: "POST",
        ...
        const data = await res.json();
        const items = (data.ekonomickeSubjekty ?? []).map((s) => {
          const kod = stavKodFrom2(s);
          return {
            ico: s.ico,
            obchodniFirma: s.obchodniJmeno ?? null,
            sidloText: s.sidlo?.textovaAdresa ?? null,
            stavNazev: stavNazevFrom2(kod)
          };
        });
...
var routes;
var init_functionsRoutes_0_13781337098352953 = __esm({
  "../.wrangler/tmp/pages-E818ob/functionsRoutes-0.13781337098352953.mjs"() {
    "use strict";
    init_ico();
    init_cap_reset();
    init_search();
...
// .wrangler/tmp/bundle-IVTQf2/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-IVTQf2/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.7402068381776428.js.map
